*------------------------------------------------------------*;
* Neural19: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBVar;
*------------------------------------------------------------*;
* Neural19: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_Neural19
dmdbcat=WORK.Neural19_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* Neural19: Interval Input Variables Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    Year_Birth
%mend INTINPUTS;
*------------------------------------------------------------* ;
* Neural19: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;
...
;
*------------------------------------------------------------*;
proc neural data=EM_Neural19 dmdbcat=WORK.Neural19_DMDB
...
Prelim 5 preiter=10
pretime=3600
Outest=EMWS8.Neural19_PRELIM_OUTEST
;
save network=EMWS8.Neural19_NETWORK.dm_neural;
train Maxiter=50
maxtime=14400
Outest=EMWS8.Neural19_outest estiter=1
Outfit=EMWS8.Neural19_OUTFIT
;
run;
quit;
proc sort data=EMWS8.Neural19_OUTFIT(where=(_iter_ ne . and _NAME_="OVERALL")) out=fit_Neural19;
by _VAVERR_;
run;
%GLOBAL ITER;
data _null_;
set fit_Neural19(obs=1);
call symput('ITER',put(_ITER_, 6.));
run;
data EMWS8.Neural19_INITIAL;
set EMWS8.Neural19_outest(where=(_ITER_ eq &ITER and _OBJ_ ne .));
run;
*------------------------------------------------------------*;
...
proc neural data=EM_Neural19 dmdbcat=WORK.Neural19_DMDB
validdata = EMWS8.BINNING_VALIDATE
network = EMWS8.Neural19_NETWORK.dm_neural
random=12345
;
nloptions noprint;
performance alldetails noutilfile;
initial inest=EMWS8.Neural19_INITIAL;
train tech=NONE;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural19\SCORECODE.sas"
group=Neural19
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural19\RESIDUALSCORECODE.sas"
group=Neural19
residual
;
;
...
run;
quit;
data EMWS8.Neural19_OUTFIT;
merge WORK.FIT1 WORK.FIT2;
run;
data EMWS8.Neural19_EMESTIMATE;
set EMWS8.Neural19_outest;
