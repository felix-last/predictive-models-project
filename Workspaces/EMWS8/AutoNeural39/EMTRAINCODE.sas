*------------------------------------------------------------*;
* AutoNeural39: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural39: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural39
dmdbcat=WORK.AutoNeural39_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural39: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Age Frq Income Kidhome MntFishProducts MntFruits MntGoldProds
   MntSweetProducts MonthsAsCustomer NumDealsPurchases NumWebVisitsMonth RFMstat
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural39: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;
...
%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural39: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Education Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural39: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural39 dmdbcat=WORK.AutoNeural39_DMDB
...
run;
*;
data EMWS8.AutoNeural39_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural39_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural39_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural39_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=877 maxiter=5
tech = Default;
;
run;
*;
...
connect H1x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural39_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural39\SCORECODE.sas"
group=AutoNeural39
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural39\RESIDUALSCORECODE.sas"
group=AutoNeural39
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural39_OUTKEY;
...
set EMWS8.AutoNeural39_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural39_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural39_WEIGHTS;
set EMWS8.AutoNeural39_WEIGHTS;
