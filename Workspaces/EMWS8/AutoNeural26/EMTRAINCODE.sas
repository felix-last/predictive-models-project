*------------------------------------------------------------*;
* AutoNeural26: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural26: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
    Year_Birth
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural26: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural26
dmdbcat=WORK.AutoNeural26_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural26: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    Year_Birth
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural26: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural26: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
...
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural26: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural26 dmdbcat=WORK.AutoNeural26_DMDB
...
run;
*;
data EMWS8.AutoNeural26_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural26_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural26_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural26_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=1768 maxiter=5
tech = Default;
;
run;
*;
...
;
*;
initial inest= EMWS8.AutoNeural26_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural26\SCORECODE.sas"
group=AutoNeural26
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural26\RESIDUALSCORECODE.sas"
group=AutoNeural26
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural26_OUTKEY;
...
set EMWS8.AutoNeural26_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural26_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural26_WEIGHTS;
set EMWS8.AutoNeural26_WEIGHTS;
