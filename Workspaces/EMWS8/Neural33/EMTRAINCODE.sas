*------------------------------------------------------------*;
* Neural33: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBVar;
*------------------------------------------------------------*;
* Neural33: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_Neural33
dmdbcat=WORK.Neural33_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* Neural33: Interval Input Variables Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    Income Kidhome MntFishProducts MntFruits MntGoldProds MntMeatProducts
   MntSweetProducts MntWines NumCatalogPurchases NumDealsPurchases
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* Neural33: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;
    AcceptedCmp1 AcceptedCmp2 AcceptedCmp3 AcceptedCmp4 AcceptedCmp5 Complain
%mend BININPUTS;
*------------------------------------------------------------* ;
* Neural33: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Education Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* Neural33: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
;
*------------------------------------------------------------*;
proc neural data=EM_Neural33 dmdbcat=WORK.Neural33_DMDB
...
Prelim 5 preiter=10
pretime=3600
Outest=EMWS8.Neural33_PRELIM_OUTEST
;
save network=EMWS8.Neural33_NETWORK.dm_neural;
train Maxiter=50
maxtime=14400
Outest=EMWS8.Neural33_outest estiter=1
Outfit=EMWS8.Neural33_OUTFIT
;
run;
quit;
proc sort data=EMWS8.Neural33_OUTFIT(where=(_iter_ ne . and _NAME_="OVERALL")) out=fit_Neural33;
by _VAVERR_;
run;
%GLOBAL ITER;
data _null_;
set fit_Neural33(obs=1);
call symput('ITER',put(_ITER_, 6.));
run;
data EMWS8.Neural33_INITIAL;
set EMWS8.Neural33_outest(where=(_ITER_ eq &ITER and _OBJ_ ne .));
run;
*------------------------------------------------------------*;
...
proc neural data=EM_Neural33 dmdbcat=WORK.Neural33_DMDB
validdata = EMWS8.Meta2_VALIDATE
network = EMWS8.Neural33_NETWORK.dm_neural
random=12345
;
nloptions noprint;
performance alldetails noutilfile;
initial inest=EMWS8.Neural33_INITIAL;
train tech=NONE;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural33\SCORECODE.sas"
group=Neural33
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural33\RESIDUALSCORECODE.sas"
group=Neural33
residual
;
;
...
run;
quit;
data EMWS8.Neural33_OUTFIT;
merge WORK.FIT1 WORK.FIT2;
run;
data EMWS8.Neural33_EMESTIMATE;
set EMWS8.Neural33_outest;
