*------------------------------------------------------------*;
* AutoNeural43: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* AutoNeural43: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DepVar(DESC) Marital_Status(ASC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural43: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural43: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural43
dmdbcat=WORK.AutoNeural43_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural43: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Frq Income Mnt MntGoldProds MntMeatProducts
   NumCatalogPurchases NumDealsPurchases NumWebVisitsMonth RFMstat Recency
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural43: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural43: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural43: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural43 dmdbcat=WORK.AutoNeural43_DMDB
...
run;
*;
data EMWS8.AutoNeural43_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural43_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural43_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural43_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=827 maxiter=50
tech = Default;
;
run;
*;
...
train estiter=1 outest=_anest outfit=_anfit maxtime=788 maxiter=50
tech = Default;
;
run;
*;
...
train estiter=1 outest=_anest outfit=_anfit maxtime=773 maxiter=50
tech = Default;
;
run;
*;
...
train estiter=1 outest=_anest outfit=_anfit maxtime=754 maxiter=5
tech = Default;
;
run;
*;
...
connect H3x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural43_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural43\SCORECODE.sas"
group=AutoNeural43
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural43\RESIDUALSCORECODE.sas"
group=AutoNeural43
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural43_OUTKEY;
...
set EMWS8.AutoNeural43_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural43_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural43_WEIGHTS;
set EMWS8.AutoNeural43_WEIGHTS;
