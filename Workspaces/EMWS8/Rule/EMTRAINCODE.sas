...
%mend EM_TREEVARS;
*------------------------------------------------------------* ;
* Rule: Tree Targets Macro ;
*------------------------------------------------------------* ;
%macro EM_TREETARGETS;
    DepVar
%mend EM_TREETARGETS;
data WORK.EM_Rule / view=WORK.EM_Rule;
set EMWS8.Meta_TRAIN(keep=%EM_TREEVARS %EM_TREETARGETS);
run;
*------------------------------------------------------------* ;
* Rule: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Age Frq Income Kidhome Mnt MntFishProducts MntFruits
   MntGoldProds MntMeatProducts MntSweetProducts MntWines MonthsAsCustomer
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* Rule: Binary and Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
...
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* Rule: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;

%mend ORDINPUTS;
*------------------------------------------------------------*;
* Rule: Run ARBOR procedure;
*------------------------------------------------------------*;
proc arbor data=WORK.EM_Rule
Leafsize=50
Mincatsize = 5
Maxdepth=4
Missing=USEINSEARCH
...
save
MODEL=WORK.OUTTREE_TREE_22N934O
SEQUENCE=WORK.OUTSEQ_TREE_22N934O
IMPORTANCE=WORK.OUTIMPORT_TREE_22N934O
NODESTAT=WORK.OUTNODES_TREE_22N934O
SUMMARY=WORK.OUTSUMMARY_TREE_22N934O
STATSBYNODE=WORK.OUTSTATS_TREE_22N934O
Pathlistnonmissing = WORK.OUTPATH_TREE_22N934O
Rules = WORK.OUTRULES_TREE_22N934O
;
...
residual
;
score data=EMWS8.Meta_TRAIN out=_ptrain
;
score data=EMWS8.Meta_VALIDATE out=_pvalid
;
run;
quit;
data WORK.OUTIMPORT_TREE_22N934O;
set WORK.OUTIMPORT_TREE_22N934O;
...
MntMeatProducts MntSweetProducts MntWines MonthsAsCustomer NumCatalogPurchases
NumDealsPurchases NumDistPurchases NumStorePurchases NumWebPurchases
NumWebVisitsMonth RFMstat RMntFrq Recency Teenhome _bin);
run;
*------------------------------------------------------------* ;
* Rule: DMDBClass Macro ;
*------------------------------------------------------------* ;
...
quit;
*------------------------------------------------------------*;
* Rule: Run DMREG procedure;
*------------------------------------------------------------*;
proc dmreg data=EM_DMREG dmdbcat=WORK.Rule_DMDB
validata = work._valid
outest = WORK.DMREG_0M3J979_OUTEST
outterms = WORK.DMREG_0M3J979_OUTTERMS
outmap= WORK.DMREG_0M3J979_MAP namelen=200
;
class
_bin
AcceptedCmp1
...
proc dmreg data=EM_DMREG dmdbcat=WORK.Rule_DMDB
validata = work._valid
outest = WORK.DMREG_2650AFG_OUTEST
outterms = WORK.DMREG_2650AFG_OUTTERMS
outmap= WORK.DMREG_2650AFG_MAP namelen=200
;
class
_bin
AcceptedCmp1
...
set work._train(keep=
AcceptedCmp1 AcceptedCmp2 AcceptedCmp3 AcceptedCmp4 AcceptedCmp5
AcceptedCmpTotal Age DepVar Education Frq HigherEducationBinary Income Kidhome
Marital_Status Mnt MntFishProducts MntFruits MntGoldProds MntMeatProducts
...
%macro DMDBClass;
    AcceptedCmp1(ASC) AcceptedCmp2(ASC) AcceptedCmp3(ASC) AcceptedCmp4(ASC)
   AcceptedCmp5(ASC) DepVar(DESC) Education(ASC) HigherEducationBinary(ASC)
   Marital_Status(ASC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* Rule: DMDBVar Macro ;
...
* Rule: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_Rule
dmdbcat=WORK.Rule_DMDB
maxlevel = 513
;
class %DMDBClass;
...
quit;
*------------------------------------------------------------* ;
* Rule: Interval Input Variables Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Age Frq Income Kidhome Mnt MntFishProducts MntFruits
   MntGoldProds MntMeatProducts MntSweetProducts MntWines MonthsAsCustomer
...
;
*------------------------------------------------------------*;
proc neural data=EM_Rule dmdbcat=WORK.Rule_DMDB
...
train Maxiter=20
maxtime=14400
Outest=WORK.Rule_outest estiter=1
Outfit=_rnfit
;
run;
quit;
proc sort data=_rnfit(where=(_iter_ ne . and _NAME_="OVERALL")) out=fit_Rule;
by _VAVERR_;
run;
%GLOBAL ITER;
data _null_;
...
run;
data WORK.INITIAL_EMNEURAL_3O2EZJZ;
set WORK.Rule_outest(where=(_ITER_ eq &ITER and _OBJ_ ne .));
run;
*------------------------------------------------------------*;
...
nloptions noprint;
performance alldetails noutilfile;
initial inest=WORK.INITIAL_EMNEURAL_3O2EZJZ;
train tech=NONE;
...
outkey=WORK.OUTKEY_EMNEURAL_3O2EZJZ;
score data=_valid out=_pvalid
outkey=WORK.OUTKEY_EMNEURAL_3O2EZJZ;
run;
quit;
data _rnest;
set WORK.Rule_outest;
