*------------------------------------------------------------*;
* Neural32: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_Neural32
dmdbcat=WORK.Neural32_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
;
*------------------------------------------------------------*;
proc neural data=EM_Neural32 dmdbcat=WORK.Neural32_DMDB
...
Prelim 5 preiter=10
pretime=3600
Outest=EMWS8.Neural32_PRELIM_OUTEST
;
save network=EMWS8.Neural32_NETWORK.dm_neural;
train Maxiter=50
maxtime=14400
Outest=EMWS8.Neural32_outest estiter=1
Outfit=EMWS8.Neural32_OUTFIT
;
run;
quit;
proc sort data=EMWS8.Neural32_OUTFIT(where=(_iter_ ne . and _NAME_="OVERALL")) out=fit_Neural32;
by _VAVERR_;
run;
%GLOBAL ITER;
data _null_;
set fit_Neural32(obs=1);
call symput('ITER',put(_ITER_, 6.));
run;
data EMWS8.Neural32_INITIAL;
set EMWS8.Neural32_outest(where=(_ITER_ eq &ITER and _OBJ_ ne .));
run;
*------------------------------------------------------------*;
...
proc neural data=EM_Neural32 dmdbcat=WORK.Neural32_DMDB
validdata = EMWS8.Meta2_VALIDATE
network = EMWS8.Neural32_NETWORK.dm_neural
random=12345
;
nloptions noprint;
performance alldetails noutilfile;
initial inest=EMWS8.Neural32_INITIAL;
train tech=NONE;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural32\SCORECODE.sas"
group=Neural32
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural32\RESIDUALSCORECODE.sas"
group=Neural32
residual
;
;
...
run;
quit;
data EMWS8.Neural32_OUTFIT;
merge WORK.FIT1 WORK.FIT2;
run;
data EMWS8.Neural32_EMESTIMATE;
set EMWS8.Neural32_outest;
