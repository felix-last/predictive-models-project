*------------------------------------------------------------*;
* AutoNeural34: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* AutoNeural34: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DepVar(DESC) Marital_Status(ASC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural34: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural34: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural34
dmdbcat=WORK.AutoNeural34_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural34: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Frq Mnt MntMeatProducts NumCatalogPurchases NumDealsPurchases
   NumWebVisitsMonth Recency
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural34: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural34: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural34: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural34 dmdbcat=WORK.AutoNeural34_DMDB
...
run;
*;
data EMWS8.AutoNeural34_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural34_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural34_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural34_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=866 maxiter=5
tech = Default;
;
run;
*;
...
connect H2x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural34_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural34\SCORECODE.sas"
group=AutoNeural34
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural34\RESIDUALSCORECODE.sas"
group=AutoNeural34
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural34_OUTKEY;
...
set EMWS8.AutoNeural34_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural34_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural34_WEIGHTS;
set EMWS8.AutoNeural34_WEIGHTS;
