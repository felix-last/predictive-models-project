*------------------------------------------------------------*;
* AutoNeural30: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* AutoNeural30: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DepVar(DESC) G_Marital_Status(ASC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural30: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural30: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural30
dmdbcat=WORK.AutoNeural30_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural30: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Frq Income Mnt MntGoldProds MntMeatProducts MntWines
   NumDistPurchases RFMstat RMntFrq Recency
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural30: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural30: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    G_Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural30: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural30 dmdbcat=WORK.AutoNeural30_DMDB
...
run;
*;
data EMWS8.AutoNeural30_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural30_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural30_HISTORY;
...
prelim 6 outest=_anpre pretime=150 preiter=25
tech = Default
;
*;
title9 " ";
...
*;
initial
inest = EMWS8.AutoNeural30_ESTDS bylabel;
;
*;
title9 " ";
...
prelim 7 outest=_anpre pretime=127 preiter=25
tech = Default
inest = EMWS8.AutoNeural30_ESTDS bylabel
;
*;
...
prelim 7 outest=_anpre pretime=126 preiter=25
tech = Default
inest = EMWS8.AutoNeural30_ESTDS bylabel
;
*;
...
train estiter=1 outest=_anest outfit=_anfit maxtime=884 maxiter=5
tech = Default;
;
run;
*;
...
connect H4x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural30_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural30\SCORECODE.sas"
group=AutoNeural30
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural30\RESIDUALSCORECODE.sas"
group=AutoNeural30
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural30_OUTKEY;
...
set EMWS8.AutoNeural30_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural30_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural30_WEIGHTS;
set EMWS8.AutoNeural30_WEIGHTS;
