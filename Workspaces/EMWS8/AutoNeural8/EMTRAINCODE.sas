*------------------------------------------------------------*;
* AutoNeural8: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural8: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural8
dmdbcat=WORK.AutoNeural8_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural8: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Age Frq Income Kidhome Mnt MntFishProducts MntFruits
   MntGoldProds MntMeatProducts MntSweetProducts MntWines MonthsAsCustomer
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural8: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;
...
%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural8: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Education Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural8: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural8 dmdbcat=WORK.AutoNeural8_DMDB
...
proc print data=EMWS8.AutoNeural8_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural8_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural8_ESTDS bylabel;
;
*;
title9 " ";
...
data EMWS8.AutoNeural8_OUTFIT;
set _anfit;
if _ITER_ eq 33 and _NAME_ eq "OVERALL" then do;
output;
stop;
end;
run;
...
prelim 12 outest=_anpre pretime=70 preiter=25
tech = Default
inest = EMWS8.AutoNeural8_ESTDS bylabel
;
*;
...
train estiter=1 outest=_anest outfit=_anfit maxtime=849 maxiter=5
tech = Default;
;
run;
*;
...
connect H1x2_ DepVar;
*;
initial inest= EMWS8.AutoNeural8_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural8\SCORECODE.sas"
group=AutoNeural8
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural8\RESIDUALSCORECODE.sas"
group=AutoNeural8
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural8_OUTKEY;
...
set EMWS8.AutoNeural8_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural8_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural8_WEIGHTS;
set EMWS8.AutoNeural8_WEIGHTS;
