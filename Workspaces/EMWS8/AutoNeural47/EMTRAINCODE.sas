*------------------------------------------------------------*;
* AutoNeural47: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural47: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural47
dmdbcat=WORK.AutoNeural47_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural47: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    Income Kidhome MntFishProducts MntFruits MntGoldProds MntMeatProducts
   MntSweetProducts MntWines NumCatalogPurchases NumDealsPurchases
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural47: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;
    AcceptedCmp1 AcceptedCmp2 AcceptedCmp3 AcceptedCmp4 AcceptedCmp5 Complain
%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural47: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Education Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural47: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural47 dmdbcat=WORK.AutoNeural47_DMDB
...
run;
*;
data EMWS8.AutoNeural47_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural47_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural47_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural47_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=834 maxiter=5
tech = Default;
;
run;
*;
...
connect H2x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural47_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural47\SCORECODE.sas"
group=AutoNeural47
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural47\RESIDUALSCORECODE.sas"
group=AutoNeural47
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural47_OUTKEY;
...
set EMWS8.AutoNeural47_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural47_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural47_WEIGHTS;
set EMWS8.AutoNeural47_WEIGHTS;
