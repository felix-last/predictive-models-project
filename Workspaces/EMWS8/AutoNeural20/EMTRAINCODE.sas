*------------------------------------------------------------*;
* AutoNeural20: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural20: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
    Year_Birth
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural20: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural20
dmdbcat=WORK.AutoNeural20_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural20: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    Year_Birth
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural20: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural20: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
...
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural20: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural20 dmdbcat=WORK.AutoNeural20_DMDB
...
run;
*;
data EMWS8.AutoNeural20_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural20_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural20_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural20_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=1709 maxiter=5
tech = Default;
;
run;
*;
...
;
*;
initial inest= EMWS8.AutoNeural20_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural20\SCORECODE.sas"
group=AutoNeural20
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural20\RESIDUALSCORECODE.sas"
group=AutoNeural20
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural20_OUTKEY;
...
set EMWS8.AutoNeural20_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural20_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural20_WEIGHTS;
set EMWS8.AutoNeural20_WEIGHTS;
