*------------------------------------------------------------*;
* AutoNeural31: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
quit;
*------------------------------------------------------------* ;
* AutoNeural31: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Mnt MntMeatProducts NumCatalogPurchases RFMstat Recency
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural31: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural31: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;

%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural31: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural31 dmdbcat=WORK.AutoNeural31_DMDB
...
run;
*;
data EMWS8.AutoNeural31_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural31_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural31_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural31_ESTDS bylabel;
;
*;
title9 " ";
...
train estiter=1 outest=_anest outfit=_anfit maxtime=891 maxiter=5
tech = Default;
;
run;
*;
...
connect H1x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural31_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural31\SCORECODE.sas"
group=AutoNeural31
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural31\RESIDUALSCORECODE.sas"
group=AutoNeural31
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural31_OUTKEY;
...
set EMWS8.AutoNeural31_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural31_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural31_WEIGHTS;
set EMWS8.AutoNeural31_WEIGHTS;
