*------------------------------------------------------------*;
* Neural37: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
%mend DMDBVar;
*------------------------------------------------------------*;
* Neural37: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_Neural37
dmdbcat=WORK.Neural37_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* Neural37: Interval Input Variables Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Age Frq Income Kidhome MntFishProducts MntFruits MntGoldProds
   MntSweetProducts MonthsAsCustomer NumDealsPurchases NumWebVisitsMonth RFMstat
...
%mend BININPUTS;
*------------------------------------------------------------* ;
* Neural37: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Education Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* Neural37: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
;
*------------------------------------------------------------*;
proc neural data=EM_Neural37 dmdbcat=WORK.Neural37_DMDB
...
Prelim 5 preiter=10
pretime=3600
Outest=EMWS8.Neural37_PRELIM_OUTEST
;
save network=EMWS8.Neural37_NETWORK.dm_neural;
train Maxiter=50
maxtime=14400
Outest=EMWS8.Neural37_outest estiter=1
Outfit=EMWS8.Neural37_OUTFIT
;
run;
quit;
proc sort data=EMWS8.Neural37_OUTFIT(where=(_iter_ ne . and _NAME_="OVERALL")) out=fit_Neural37;
by _VAVERR_;
run;
%GLOBAL ITER;
data _null_;
set fit_Neural37(obs=1);
call symput('ITER',put(_ITER_, 6.));
run;
data EMWS8.Neural37_INITIAL;
set EMWS8.Neural37_outest(where=(_ITER_ eq &ITER and _OBJ_ ne .));
run;
*------------------------------------------------------------*;
...
proc neural data=EM_Neural37 dmdbcat=WORK.Neural37_DMDB
validdata = EMWS8.Meta3_VALIDATE
network = EMWS8.Neural37_NETWORK.dm_neural
random=12345
;
nloptions noprint;
performance alldetails noutilfile;
initial inest=EMWS8.Neural37_INITIAL;
train tech=NONE;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural37\SCORECODE.sas"
group=Neural37
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\Neural37\RESIDUALSCORECODE.sas"
group=Neural37
residual
;
;
...
run;
quit;
data EMWS8.Neural37_OUTFIT;
merge WORK.FIT1 WORK.FIT2;
run;
data EMWS8.Neural37_EMESTIMATE;
set EMWS8.Neural37_outest;
