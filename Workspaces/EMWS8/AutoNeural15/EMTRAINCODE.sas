*------------------------------------------------------------*;
* AutoNeural15: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* AutoNeural15: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DepVar(DESC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural15: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural15: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural15
dmdbcat=WORK.AutoNeural15_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural15: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    PC_1 PC_10 PC_11 PC_12 PC_13 PC_14 PC_15 PC_16 PC_17 PC_2 PC_3 PC_4 PC_5 PC_6
   PC_7 PC_8 PC_9 Year_Birth
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural15: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural15: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;

%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural15: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural15 dmdbcat=WORK.AutoNeural15_DMDB
...
run;
*;
data EMWS8.AutoNeural15_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural15_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural15_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural15_ESTDS bylabel;
;
*;
title9 " ";
...
connect H1x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural15_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural15\SCORECODE.sas"
group=AutoNeural15
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural15\RESIDUALSCORECODE.sas"
group=AutoNeural15
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural15_OUTKEY;
...
set EMWS8.AutoNeural15_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural15_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural15_WEIGHTS;
set EMWS8.AutoNeural15_WEIGHTS;
