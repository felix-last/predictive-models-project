*------------------------------------------------------------*;
* AutoNeural46: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* AutoNeural46: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DepVar(DESC) Marital_Status(ASC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural46: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural46: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural46
dmdbcat=WORK.AutoNeural46_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural46: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    AcceptedCmpTotal Frq Income Mnt MntGoldProds MntMeatProducts
   NumCatalogPurchases NumDealsPurchases NumWebVisitsMonth RFMstat Recency
...
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural46: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural46: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;
    Marital_Status
%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural46: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural46 dmdbcat=WORK.AutoNeural46_DMDB
...
run;
*;
data EMWS8.AutoNeural46_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural46_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural46_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural46_ESTDS bylabel;
;
*;
title9 " ";
...
data EMWS8.AutoNeural46_OUTFIT;
set _anfit;
if _ITER_ eq 34 and _NAME_ eq "OVERALL" then do;
output;
stop;
end;
run;
...
connect H1x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural46_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural46\SCORECODE.sas"
group=AutoNeural46
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural46\RESIDUALSCORECODE.sas"
group=AutoNeural46
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural46_OUTKEY;
...
set EMWS8.AutoNeural46_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural46_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural46_WEIGHTS;
set EMWS8.AutoNeural46_WEIGHTS;
