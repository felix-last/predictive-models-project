*------------------------------------------------------------*;
* AutoNeural16: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DepVar;
  length   DepVar                           $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* AutoNeural16: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DepVar(DESC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* AutoNeural16: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* AutoNeural16: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_AutoNeural16
dmdbcat=WORK.AutoNeural16_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------* ;
* AutoNeural16: Interval Inputs Macro ;
*------------------------------------------------------------* ;
%macro INTINPUTS;
    PC_1 PC_10 PC_11 PC_12 PC_13 PC_14 PC_15 PC_16 PC_17 PC_2 PC_3 PC_4 PC_5 PC_6
   PC_7 PC_8 PC_9 Year_Birth
%mend INTINPUTS;
*------------------------------------------------------------* ;
* AutoNeural16: Binary Inputs Macro ;
*------------------------------------------------------------* ;
%macro BININPUTS;

%mend BININPUTS;
*------------------------------------------------------------* ;
* AutoNeural16: Nominal Inputs Macro ;
*------------------------------------------------------------* ;
%macro NOMINPUTS;

%mend NOMINPUTS;
*------------------------------------------------------------* ;
* AutoNeural16: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro ORDINPUTS;
...
*------------------------------------------------------------*;
*;
proc neural data=EM_AutoNeural16 dmdbcat=WORK.AutoNeural16_DMDB
...
run;
*;
data EMWS8.AutoNeural16_OUTFIT;
set _anfit;
...
proc print data=EMWS8.AutoNeural16_OUTFIT noobs;
var _iter_ _aic_ _averr_ _misc_
_vaverr_ _vmisc_
;
run;
...
data EMWS8.AutoNeural16_HISTORY;
...
*;
initial
inest = EMWS8.AutoNeural16_ESTDS bylabel;
;
*;
title9 " ";
...
connect H2x1_ DepVar;
*;
initial inest= EMWS8.AutoNeural16_ESTDS bylabel;
train tech=none;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural16\SCORECODE.sas"
group=AutoNeural16
;
;
code file="C:\\predictive-models-project\Workspaces\EMWS8\AutoNeural16\RESIDUALSCORECODE.sas"
group=AutoNeural16
residual
;
;
...
outfit=WORK.FIT1
role=TRAIN
outkey=EMWS8.AutoNeural16_OUTKEY;
...
set EMWS8.AutoNeural16_ESTDS;
run;
proc sort;
by _name_;
run;
proc transpose data=tempweight out=EMWS8.AutoNeural16_WEIGHTS;
by _name_;
run;
data EMWS8.AutoNeural16_WEIGHTS;
set EMWS8.AutoNeural16_WEIGHTS;
